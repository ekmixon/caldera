...
class EventService(EventServiceInterface, BaseService):
    ...
    def __init__(self):
        ...
        self.ws_uri = f"ws://{self.get_config('app.contact.websocket')}"
    ...
    async def notify_global_event_listeners(self, event, **callback_kwargs):
        ...
        for c in self.global_listeners:
            try:
                ...
                self.log.error(f"Global callback error: {e}", exc_info=True)
    ...
    async def fire_event(self, exchange=None, queue=None, timestamp=True, **callback_kwargs):
        ...
        metadata = {}
        if timestamp:
            metadata |= dict(timestamp=datetime.datetime.now().timestamp())
        callback_kwargs |= dict(metadata=metadata)
